...
An example Weights & Biases config for a sweep pruning LLaMA-3-8B to 2:4 sparsity with Wanda and reconstructing with block size 1/2 (-1 in this code base) can be found in [`example_config.txt`](example_config.txt). The model with the best WikiText-2 perplexity is saved in `checkpointdir/<sweep_id>`. An example config for evaluating the best model found by the sweep is given in [`example_eval_config.txt`](example_eval_config.txt).
...
- [`check_act_dist_main.py`](check_act_dist_main.py): Configures starts and configures the following script.
- [`check_act_dist.py`](check_act_dist.py): Contains a class that measures the distances between the activations of given model checkpoints.
